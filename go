...
local idle = "124242129571162"
local walk = "133138659569444"
local qAnim = "138282513302553"
-- Animation to play when Q is pressed 

local Character = Empyrean.Rig
local Humanoid = Character:WaitForChild("Humanoid")
local UserInputService = game:GetService("UserInputService")

-- Method gotten from shownape
local function playanim(id, speed, prio, weight)
	local keyframe = game:GetService("InsertService"):LoadLocalAsset("rbxassetid://" .. id)
	local localid = game:GetService("AnimationClipProvider"):RegisterActiveAnimationClip(keyframe)
...
Humanoid.Running:Connect(function(Speed)
	if Speed > 1 and not IsWalking then
		IsWalking = true
		IsIdle = false
		playanim(walk, 1, 5, 1)
	elseif Speed < 1 and not IsIdle then
		IsWalking = false
		IsIdle = true
		playanim(idle, 1, 5, 1)
	end
...
UserInputService.InputBegan:Connect(function(input, gameProcessedEvent)
    -- Check if the game hasn't processed the input (this prevents triggering while the UI is open)
    if not gameProcessedEvent then
        -- Check if the left mouse button (MouseButton1) was clicked
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            -- Call the playanim function (with your parameters)
            playanim(qAnim, 1, 5, 1)
        end
    end
end)
